...
try:
    ...
    class _DummyCredentials:
        ...
        def from_service_account_file(*args, **kwargs):
            raise RuntimeError("service_account not available in this environment")
...
def load_to_google_sheets(
    df, credentials_path="google-sheets-api.json", spreadsheet_id=None
):
    """
    ...
    try:
        ...
        if not os.path.exists(credentials_path):
            print(
                f"Warning: Credentials file {credentials_path} not found. Skipping Google Sheets load."
            )
            return False
        ...
        service.spreadsheets().values().clear(
            spreadsheetId=spreadsheet_id, range="Sheet1!A1:Z1500"
        ).execute()
        ...
        print(
            f"Successfully uploaded to Google Sheets. Updated {result.get('updatedCells')} cells."
        )
        return True
    except Exception as e:
